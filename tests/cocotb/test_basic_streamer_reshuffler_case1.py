...
# found in ./util/cfg/streamer_cfg_reshuffler_case1.hjson
#
# Sequence of tests:
...
BASE_PTR_M1 = NUM_BANKS * math.ceil(LOOP_COUNT_0 * ALIGN_ELEMS / NUM_BANKS) + OFFSET
...
async def basic_streamer_dut(dut):
    ...
    for i in range(LOOP_COUNT_0):
        ...
        cocotb.log.info(f"Iter-{i}, Reading address from TCDM is {rd_addr_read_val}")
        # snax_util.comp_and_assert(golden_list[i][j], rd_addr_read_val)
        wr_addr_read_val = int(dut.tcdm_req_addr_o[1].value) 
        cocotb.log.info(f"Iter-{i}, Writing address to TCDM is {wr_addr_read_val}")
...
def test_basic_streamer_reshuffler(simulator, waves):
    ...
    module = "test_basic_streamer_reshuffler_case1"
