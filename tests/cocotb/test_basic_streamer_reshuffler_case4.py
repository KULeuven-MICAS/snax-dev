...
# found in ./util/cfg/streamer_cfg_reshuffler_case4.hjson
#
# Sequence of tests:
...
BASE_PTR_M1 = NUM_BANKS * math.ceil(LOOP_COUNT_0 * LOOP_COUNT_1 * LOOP_COUNT_2 * ALIGN_ELEMS / NUM_BANKS) + OFFSET
    ...
    for i in range(LOOP_COUNT_2):
        for j in range(LOOP_COUNT_1):
            for k in range(LOOP_COUNT_0):
                iter_count = i * LOOP_COUNT_1 * LOOP_COUNT_0 + j * LOOP_COUNT_0 + k
...
def test_basic_streamer_reshuffler(simulator, waves):
    ...
    module = "test_basic_streamer_reshuffler_case4"
