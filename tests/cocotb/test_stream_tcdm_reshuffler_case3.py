...
async def stream_tcdm_dut(dut):
    ...
    narrow_golden_list = snax_util.gen_rand_int_list(
        int(LOOP_COUNT_0 * LOOP_COUNT_1), MIN_VAL, 255
    )
    ...
    for i in range(LOOP_COUNT_1):
        for j in range(LOOP_COUNT_0):
            ...
            cocotb.log.info(">> Gold_list:     %s", narrow_golden_list[(i * TEMPORAL_STRIDE_M0_L1 + j * TEMPORAL_STRIDE_M0_L0) // ALIGN_ELEMS])
            cocotb.log.info(">> Read_Stream_0: %s", read_stream_0)
            snax_util.comp_and_assert(narrow_golden_list[(i * TEMPORAL_STRIDE_M0_L1 + j * TEMPORAL_STRIDE_M0_L0) // ALIGN_ELEMS], read_stream_0)
            await snax_util.clock_and_wait(dut)            
    ...
    await snax_util.reg_write(dut, CSR_LOOP_COUNT_1, LOOP_COUNT_0 * LOOP_COUNT_1)
    ...
    # # of the previously set registers
    # reg_val = await snax_util.reg_read(dut, CSR_LOOP_COUNT_0)
    # snax_util.comp_and_assert(1, reg_val)
    # reg_val = await snax_util.reg_read(dut, CSR_LOOP_COUNT_1)
    # snax_util.comp_and_assert(LOOP_COUNT_0 * LOOP_COUNT_1, reg_val)
    ...
    for i in range(LOOP_COUNT_1):
        for j in range(LOOP_COUNT_0):
            ...
            cocotb.log.info(">> Gold_list:     %s", narrow_writer_golden_list[(i * TEMPORAL_STRIDE_M1_L1 + j * TEMPORAL_STRIDE_M1_L0) // ALIGN_ELEMS])
            cocotb.log.info(">> Read_Stream_0: %s", read_stream_0)
            # Streamed data should be consistent
            snax_util.comp_and_assert(narrow_writer_golden_list[(i * TEMPORAL_STRIDE_M1_L1 + j * TEMPORAL_STRIDE_M1_L0) // ALIGN_ELEMS], read_stream_0)
            await snax_util.clock_and_wait(dut)
...
def test_stream_tcdm(simulator, waves):
    ...
    module = "test_stream_tcdm_reshuffler_case3"
